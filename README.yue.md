...
Nairobi OS 係用於喺本地硬件上運行 AI 同數據科學管道嘅基礎設施，無需支付 Python 默認工具鏈喺每個層級引入嘅開銷：

1. **Python 稅** — 數據密集型工作負載中嘅端到端內存複製、GIL 爭用同解釋器開銷。
2. **瀏覽器稅** — 當智能體（agent）工具基於瀏覽器界面構建以進行長時間、高頻率互動時產生嘅渲染延遲同通信開銷。
3. **OS 內核瓶頸** — 為桌面自動化工作負載增加延遲嘅進程調度同顯示服務器開銷（Wayland 與 X11 上下文切換）。

Nairobi OS 係一個基於 Rust 嘅基礎設施棧 — 零拷貝數據管道、硬件原生執行同用於桌面自動化嘅語義（非像素）界面 — 專為直接消除呢啲開銷而構建。本倉庫係該 Stack 嘅開源核心。
