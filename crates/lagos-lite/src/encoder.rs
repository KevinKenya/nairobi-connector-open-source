...
pub fn compress_rgba_to_jpeg(width: u32, height: u32, rgba_data: &[u8], quality: u8) -> Result<Vec<u8>, String> {
    ...
    Encoder::new(&mut output, quality)
        .encode(rgba_data, width as u16, height as u16, ColorType::Rgba)
        .map_err(|e| format!("Failed to encode JPEG: {}", e))?;

    Ok(output)
}

pub fn compress_rgba_to_png(width: u32, height: u32, rgba_data: &[u8]) -> Result<Vec<u8>, String> {
    let mut output = Vec::new();
    image::codecs::png::PngEncoder::new(&mut output)
        .write_image(rgba_data, width, height, image::ColorType::Rgba8)
