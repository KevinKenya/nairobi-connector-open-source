...
Nairobi OS 是用于在本地硬件上运行 AI 和数据科学管道的基础设施，无需支付 Python 默认工具链在每个层级引入的开销：

1. **Python 税** — 数据密集型工作负载中的端到端内存复制、GIL 争用和解释器开销。
2. **浏览器税** — 当智能体（agent）工具基于浏览器界面构建以进行长时间、高频率交互时产生的渲染延迟和通信开销。
3. **OS 内核瓶颈** — 为桌面自动化工作负载增加延迟的进程调度和显示服务器开销（Wayland 与 X11 上下文切换）。

Nairobi OS 是一个基于 Rust 的基础设施栈 — 零拷贝数据管道、硬件原生执行和用于桌面自动化的语义（非像素）界面 — 专为直接消除这些开销而构建。本仓库是该 Stack 的开源核心。
